...
def compute_psnr(a, b):  # PSNR= -10log10(MSE)
...
def compute_bpp(out_net):  # calculate bpp
    size = out_net['x_hat'].size()
    num_pixels = size[0] * size[2] * size[3]  # except the number of channel size[2]=height size[3]=width
...
def crop(x, padding):  # padding=(left,right,top,bottom)
    return F.pad(
        x,
...
def main(argv):
    ...
    if args.cuda:
        ...
        device = 'cpu'
    net = ICWEM(config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0.0, N=128, M=320)
    net = net.to(device)
    net.eval()
    count = 0
    PSNR = 0
    ...
    if args.real:
        ...
        for img_name in img_list:
            ...
            with torch.no_grad():
                ...
                out_processed_tensor = out_net["x_hat"].squeeze(0).cpu()
                out_net_tensor = out_processed_tensor * 255
                out_net_tensor = out_net_tensor.byte()
                out_net_array = out_net_tensor.numpy()
                import numpy as np
                out_net_array = np.transpose(out_net_array, (1, 2, 0))
                image = Image.fromarray(out_net_array)
                save_path = os.path.join("your_save_path/", img_name)
